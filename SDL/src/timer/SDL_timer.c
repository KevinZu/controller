...
static SDL_TimerID last_removed_timer;
static SDL_bool list_removed = SDL_FALSE; 
	...
	for ( prev = NULL, t = SDL_timers; t; t = next ) {
		...
		if ( (int)(now - t->last_alarm) > (int)ms ) {
			...
			if ( list_changed) 
			{
				...
				//xinghua 20100324
				if (list_removed && last_removed_timer == t)
				{
					break;	
				}
...
SDL_bool SDL_RemoveTimer(SDL_TimerID id)
{
	...
	for (t = SDL_timers; t; prev=t, t = t->next ) {
		if ( t == id ) {
			...
			//xinghua 20100324
			last_removed_timer = id;
			list_removed = SDL_TRUE;
			break;
		}
	}
#ifdef DEBUG_TIMERS
