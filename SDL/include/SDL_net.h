...
#endif
extern DECLSPEC int SDLCALL SDLNet_ResolveHost(IPaddress *address, const char *host, unsigned short port);
...
*/
extern DECLSPEC UDPsocket SDLCALL SDLNet_UDP_Open(unsigned short port);
...
*/
extern DECLSPEC int SDLCALL SDLNet_CheckSockets(SDLNet_SocketSet set, unsigned int timeout);
...
static __inline__ void _SDLNet_Write16(unsigned short value, void *areap)
{
    *(unsigned short *)areap = SDL_SwapBE16(value);
}

static __inline__ void _SDLNet_Write32(unsigned int value, void *areap)
{
    *(unsigned int *)areap = SDL_SwapBE32(value);
}

static __inline__ unsigned short _SDLNet_Read16(const void *areap)
{
    return SDL_SwapBE16(*(const unsigned short *)areap);
}

static __inline__ unsigned int _SDLNet_Read32(const void *areap)
{
    return SDL_SwapBE32(*(const unsigned int *)areap);
}
...
static __inline__ unsigned short _SDLNet_Read16(void *areap)
{
    unsigned char *area = (unsigned char*)areap;
    return ((unsigned short)area[0]) << 8 | ((unsigned short)area[1]);
}

static __inline__ unsigned int _SDLNet_Read32(const void *areap)
{
    const unsigned char *area = (const unsigned char*)areap;
    return ((unsigned int)area[0]) << 24 | ((unsigned int)area[1]) << 16 | ((unsigned int)area[2]) << 8 | ((unsigned int)area[3]);
}
